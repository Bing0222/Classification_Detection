...
class InvertedResidual(nn.Module):
    def __init__(self, inp, oup, stride, use_res_connect, expand_ratio=6):
        ...
        self.stride = stride
        assert stride in [1, 2]         # only support 1 or 2 stride

        self.use_res_connect = use_res_connect  # whether using jump layer link

        self.conv = nn.Sequential(
            # 1*1 inp --> inp * expand_ratio
...
class PFLDInference(nn.Module):
    ...
    def forward(self, x):  # x: 3, 112, 112
        ...
        multi_scale = torch.cat([x1, x2, x3], 1)  # connection
        landmarks = self.fc(multi_scale)  #keypoints
